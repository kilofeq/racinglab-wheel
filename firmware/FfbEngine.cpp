...
int32_t FfbEngine::ForceCalculator(
    ...
    if ((effect.state & MEFFECTSTATE_PLAYING) && ((effect.elapsedTime <= effect.duration) || (effect.duration == USB_DURATION_INFINITE)) && !ffbReportHandler->devicePaused)
    {
      ...
      switch (effect.effectType)
      {
        ...
        case USB_EFFECT_SPRING:
          //          position
          //          ReportPrint(effect);
          force += ConditionForceCalculator(effect, NormalizeRange(currentPosition, maxValue)) * springGainConfig;
          break;
        case USB_EFFECT_DAMPER:
          force += ConditionForceCalculator(effect, NormalizeRange(currentVelocity, maxVelocity)) * damperGainConfig;
          break;
        case USB_EFFECT_INERTIA:
          if (currentAcceleration < 0 and positionChange < 0) {
            force += ConditionForceCalculator(effect, abs(NormalizeRange(currentAcceleration, maxAcceleration))) * inertiaGainConfig;
          } else if (currentAcceleration < 0 and positionChange > 0) {
            force -= ConditionForceCalculator(effect, abs(NormalizeRange(currentAcceleration, maxAcceleration))) * inertiaGainConfig;
          }
          break;
        case USB_EFFECT_FRICTION:
          force += ConditionForceCalculator(effect, NormalizeRange(positionChange, maxPositionChange)) * frictionGainConfig;
          break;
  ...
  return constrain(force, -300, 300);
}
